...
export class KaFunComponent implements OnInit {
	...
	drawChart(data) {
		...
		function drawSvg() {
			...
			svg1.append('g').attr('class', 'axis_x1').attr('transform', 'translate(' + 0 + ',' + 0 + ')').call(xAxis);
			svg1.append('g').attr('class', 'axis_y1').attr('transform', 'translate(' + 0 + ',' + 0 + ')').call(yAxis);
		}
		...
		function drawRightTopLegend() {
			...
			let legendClick_g = legend_g
				.append('g')
				.attr('class', 'click-gradient-legend')
				.attr('transform', 'translate(' + 10 + ',' + t_height + ')')
				.style('cursor', 'pointer');

			legendClick_g.append('title').text('双击修改颜色');

			legendClick_g
				.selectAll('.legendClick_Rect')
				.data(that.colors)
				.enter()
				.append('rect')
				.attr('width', rectWidth - 10)
			...
			legend_g
				.append('g')
				.attr('class', 'gradient_legendAxis')
		...
		function setBubble(r_legend) {
			...
			resultarr.forEach(function(d, i) {
				...
				r_sum += r_size;
				circle_legend.append('circle').attr('r', rScale(d) / 4).attr('cy', r_sum / 5 + i * space);
				circle_legend
					.append('text')
			...
			tempSvg
				...
				.on('mouseover', function(d) {
					//console.log(d.bucket);
					let tempBucket = [];
					if (d.bucket.length > 30) {
						d.bucket.forEach((d, index) => {
							if (index < 30) {
								tempBucket.push(d);
								return;
							}
