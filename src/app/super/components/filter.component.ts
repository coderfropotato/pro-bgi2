    ...
    */
	...
	confirm() {
		...
		if (this.selectType === 'range') {
			if (!this.force(valueOne) || !this.force(valueTwo)) {
				this.nodata = true;
				return;
			} else {
				this.nodata = false;
		...
		// $in 默认用逗号拼接
		if (this.selectType === '$in') valueOne = valueOne.replace(/\n|\,|\，/g, ',');
    ...
    */
	...
	_outerDeleteWithoutRequest(filterName: string, filterNamezh: string, filterType: string) {
		if (this.filterName === filterName && this.filterNamezh === filterNamezh && this.selectType === filterType) {
			...
			this.visible = false;
			// 通知表格删除 筛选条件为  filterName 类型为beforeFilterType 的 筛选对象
			this.deleteDataWithoutRequest.emit([ this.filterName, this.filterNamezh, beforeFilterType ]);
		}
	}
}
